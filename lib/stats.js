...
// Remember today's longest continuous-sitting bout (minutes) if this one beats it.
function noteLongestSit(mins) {
  if (!Number.isFinite(mins) || mins <= 0) return;
  const days = store.get("days");
  const k = todayKey();
  const d = days[k] || { goodSec: 0, badSec: 0, awaySec: 0, micros: 0, longs: 0, skipped: 0 };
  if (mins > (d.longestSitMin || 0)) {
    d.longestSitMin = Math.round(mins);
    days[k] = d;
    store.set("days", days);
  }
}
...
function summary() {
  ...
  return {
    ...
    stands: today.stands || 0,
    standPrompts: today.standPrompts || 0,
    longestSitMin: today.longestSitMin || 0,
    streak: streak(),
    last5: recentScore(5),
...
module.exports = { sample, bumpDay, noteLongestSit, summary, recentScore, dailySeries, streak, flush };
